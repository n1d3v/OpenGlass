...
namespace OpenGlass::ReflectionEffect
{
    namespace Details
    {
        inline std::atomic<bool> g_reflectionTexturePathChanged{ false };
        ...
        inline std::future<void> g_textureLoadingTask;
        inline std::mutex g_textureMutex;
    }

    inline void UpdateTexture(std::wstring_view reflectionTexturePath)
    {
        std::lock_guard<std::mutex> lock(Details::g_textureMutex);
        if (Details::g_reflectionTexturePath != reflectionTexturePath)
        {
    ...
    inline HRESULT LoadTextureAsync(ID2D1DeviceContext* context)
    {
        ...
        RETURN_IF_FAILED(wicConverter->Initialize(wicFrame.get(), GUID_WICPixelFormat32bppPBGRA, WICBitmapDitherTypeNone, nullptr, 0, WICBitmapPaletteTypeCustom));

        RETURN_IF_FAILED(Details::g_reflectionBitmapSourceEffect->SetValue(D2D1_BITMAPSOURCE_PROP_WIC_BITMAP_SOURCE, wicConverter.get()));
        RETURN_IF_FAILED(wicConverter->GetSize(&Details::g_reflectionTextureWidth, &Details::g_reflectionTextureHeight));
    ...
    )
    {
        ...
        {
            RETURN_IF_FAILED(context->CreateEffect(CLSID_D2D1BitmapSource, Details::g_reflectionBitmapSourceEffect.put()));
            RETURN_IF_FAILED(Details::g_reflectionBitmapSourceEffect->SetValue(D2D1_BITMAPSOURCE_PROP_ALPHA_MODE, D2D1_ALPHA_MODE_PREMULTIPLIED));
            RETURN_IF_FAILED(Details::g_reflectionBitmapSourceEffect->SetValue(D2D1_BITMAPSOURCE_PROP_INTERPOLATION_MODE, D2D1_INTERPOLATION_MODE_LINEAR));
            RETURN_IF_FAILED(Details::g_reflectionBitmapSourceEffect->SetValue(D2D1_PROPERTY_CACHED, TRUE));
            Details::g_reflectionTexturePathChanged = true;
        }

        if (!Details::g_cropEffect)
        {
            RETURN_IF_FAILED(context->CreateEffect(CLSID_D2D1Crop, Details::g_cropEffect.put()));
            RETURN_IF_FAILED(Details::g_cropEffect->SetValue(D2D1_CROP_PROP_BORDER_MODE, D2D1_BORDER_MODE_SOFT));
        ...
        {
            if (Details::g_textureLoadingTask.valid() && Details::g_textureLoadingTask.wait_for(std::chrono::seconds(0)) == std::future_status::ready)
            {
                Details::g_textureLoadingTask.get();
            }

            if (!Details::g_textureLoadingTask.valid())
            {
                Details::g_textureLoadingTask = std::async(std::launch::async, [context]() {
                    std::lock_guard<std::mutex> lock(Details::g_textureMutex);
        ...
        D2D1_SIZE_F size = canvasSize ? *canvasSize : context->GetSize();
        RETURN_IF_FAILED(Details::g_reflectionBitmapSourceEffect->SetValue(D2D1_BITMAPSOURCE_PROP_SCALE, D2D1::Vector2F(size.width / Details::g_reflectionTextureWidth, size.height / Details::g_reflectionTextureHeight)));

        D2D1_MATRIX_3X2_F matrix{};
        context->GetTransform(&matrix);

        D2D1_POINT_2F offset = glassOffset ? *glassOffset : D2D1::Point2F(matrix.dx, matrix.dy);
        ...
        D2D1_RECT_F cropRect{ offset.x * (1.f - parallaxIntensity), offset.y };
        ...
        RETURN_IF_FAILED(Details::g_cropEffect->SetValue(D2D1_CROP_PROP_RECT, cropRect));
        ...
        context->PushLayer(D2D1::LayerParameters1(bounds, geometry, D2D1_ANTIALIAS_MODE_ALIASED, D2D1::IdentityMatrix(), reflectionIntensity, nullptr, D2D1_LAYER_OPTIONS1_NONE), nullptr);
        context->DrawImage(Details::g_cropEffect.get(), point, cropRect);
        context->PopLayer();
    ...
    inline void Reset()
    {
        ...
        if (Details::g_textureLoadingTask.valid())
        {
            Details::g_textureLoadingTask.wait();
        }
    }
}