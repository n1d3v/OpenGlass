...
namespace OpenGlass::GlassFramework
{
    ...
    HRESULT STDMETHODCALLTYPE MyCTopLevelWindow_CloneVisualTreeForLivePreview(uDwm::CTopLevelWindow* This, bool cloneForReflection, bool reserved1, bool reserved2, uDwm::CTopLevelWindow** cloned);
...
HRESULT STDMETHODCALLTYPE GlassFramework::MyCDrawGeometryInstruction_Create(uDwm::CBaseLegacyMilBrushProxy* brush, uDwm::CBaseGeometryProxy* geometry, uDwm::CDrawGeometryInstruction** instruction)
{
    ...
    {
        auto color = Shared::g_forceAccentColorization
...
{
    if (!Shared::IsBackdropAllowed() || !This->GetData())
    {
    ...
    if (This->HasNonClientBackground())
    {
        ...
        {
            oldSystemBackdropType = *reinterpret_cast<DWORD*>(reinterpret_cast<ULONG_PTR>(This->GetData()) + 204);
            *reinterpret_cast<DWORD*>(reinterpret_cast<ULONG_PTR>(This->GetData()) + 204) = 0;
        }
        ...
        if (os::buildNumber == os::build_w11_21h2)
        {
            *reinterpret_cast<DWORD*>(reinterpret_cast<ULONG_PTR>(This->GetData()) + 204) = oldSystemBackdropType;
        }
...
{
    if (!Shared::IsBackdropAllowed() || !Shared::g_overrideAccent)
    {
    ...
    return g_CAccent_UpdateAccentPolicy_Org(This, lprc, &accentPolicy, geometry);
}
...
{
    if (!Shared::IsBackdropAllowed() || !Shared::g_overrideAccent || !This->GetHwnd())
    {
    ...
    HRESULT hr{ g_CAccent__UpdateSolidFill_Org(This, visual, color, lprc, opacity) };
...
{
    if (!Shared::IsBackdropAllowed() || !Shared::g_overrideAccent || g_accentRenderDataVisual != This)
    {
    ...
    wil::unique_hrgn region{ CreateRectRgn(static_cast<LONG>(rectangle.left), static_cast<LONG>(rectangle.top), static_cast<LONG>(rectangle.right), static_cast<LONG>(rectangle.bottom)) };
    RETURN_LAST_ERROR_IF_NULL(region);

    winrt::com_ptr<uDwm::CRgnGeometryProxy> rgnGeometry{ nullptr };
    RETURN_IF_FAILED(uDwm::ResourceHelper::CreateGeometryFromHRGN(region.get(), rgnGeometry.put()));

    winrt::com_ptr<uDwm::CSolidColorLegacyMilBrushProxy> solidBrush{ nullptr };
    RETURN_IF_FAILED(uDwm::CDesktopManager::s_pDesktopManagerInstance->GetCompositor()->CreateSolidColorLegacyMilBrushProxy(solidBrush.put()));
...
HRESULT STDMETHODCALLTYPE GlassFramework::MyCTopLevelWindow_UpdateSystemBackdropVisual(uDwm::CTopLevelWindow* This)
{
    ...
    auto oldSystemBackdropType = *reinterpret_cast<DWORD*>(reinterpret_cast<ULONG_PTR>(This->GetData()) + 204);
    *reinterpret_cast<DWORD*>(reinterpret_cast<ULONG_PTR>(This->GetData()) + 204) = 0;
    HRESULT hr{ g_CTopLevelWindow_UpdateSystemBackdropVisual_Org(This) };
    *reinterpret_cast<DWORD*>(reinterpret_cast<ULONG_PTR>(This->GetData()) + 204) = oldSystemBackdropType;
...
HRESULT STDMETHODCALLTYPE GlassFramework::MyCTopLevelWindow_CloneVisualTreeForLivePreview(uDwm::CTopLevelWindow* This, bool cloneForReflection, bool reserved1, bool reserved2, uDwm::CTopLevelWindow** cloned)
{
    auto hr = g_CTopLevelWindow_CloneVisualTreeForLivePreview_Org(This, cloneForReflection, reserved1, reserved2, cloned);

    if (SUCCEEDED(hr) && cloneForReflection)
    {
        winrt::com_ptr<uDwm::CCanvasVisual> visual{ nullptr };
        RETURN_IF_FAILED(uDwm::CCanvasVisual::Create(visual.put()));

        winrt::com_ptr<uDwm::CSolidColorLegacyMilBrushProxy> brush{ nullptr };
        RETURN_IF_FAILED(uDwm::CDesktopManager::s_pDesktopManagerInstance->GetCompositor()->CreateSolidColorLegacyMilBrushProxy(brush.put()));
        RETURN_IF_FAILED(brush->Update(1.0, D2D1::ColorF(0x000000, 0.25f)));

        RECT windowRect{};
        This->GetActualWindowRect(&windowRect, true, true, false);

        wil::unique_hrgn region{ CreateRoundRectRgn(windowRect.left, windowRect.top, windowRect.right, windowRect.bottom, Shared::g_roundRectRadius, Shared::g_roundRectRadius) };
        RETURN_LAST_ERROR_IF_NULL(region);

        winrt::com_ptr<uDwm::CRgnGeometryProxy> geometry{ nullptr };
        RETURN_IF_FAILED(uDwm::ResourceHelper::CreateGeometryFromHRGN(region.get(), geometry.put()));
        ...
        RETURN_IF_FAILED((*cloned)->GetNonClientVisual()->GetVisualCollection()->InsertRelative(visual.get(), nullptr, false, true));
    }
